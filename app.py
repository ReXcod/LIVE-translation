...
def convert_to_wav(audio_file, file_extension):
    try:
        ...
        st.error(f"Failed to convert audio to WAV: {str(e)}")
        return None
...
# Streamlit app
st.title("Language Translator with Multiple Audio Formats")
st.write("Upload an audio file (WAV, MP3, MKV, etc.), choose input/output languages, and get translated audio!")
...
# File uploader for audio input with multiple formats
upload_col, _ = st.columns(2)
with upload_col:
    uploaded_file = st.file_uploader("Choose an audio file (WAV, MP3, MKV, etc.)", type=["wav", "mp3", "mkv", "ogg"])
...
st.write("Note: Supports WAV, MP3, MKV, and more. Uses ElevenLabs with gTTS fallback. Requires ffmpeg for non-WAV formats.")
